...
create table ms_usuarios.cliente (id_cliente  serial not null, cuit varchar(11) not null, fecha_baja timestamp, habilitado_online boolean default false not null, mail varchar(255) not null, max_cuenta_corriente float8, razon_social varchar(32) not null, id_usuario int4, primary key (id_cliente));
create table ms_usuarios.empleado (id_empleado  serial not null, mail varchar(255) not null, nombre varchar(32) not null, id_usuario int4, primary key (id_empleado));
create table ms_usuarios.obra (id_obra  serial not null, descripcion varchar(128), direccion varchar(32) not null, latitud float4 not null, longitud float4 not null, superficie int4, id_cliente int4, id_tipo_obra int4, primary key (id_obra));
...
create table ms_usuarios.usuario (id_usuario  serial not null, clave varchar(32) not null, usuario varchar(20) not null, id_tipo_usuario int4, primary key (id_usuario));
alter table ms_usuarios.cliente add constraint UK_as537scmvws7al2fgsv91u4aj unique (cuit);
alter table ms_usuarios.usuario add constraint UK_i02kr8ui5pqddyd7pkm3v4jbt unique (usuario);
alter table ms_usuarios.cliente add constraint FKetx0tojxf5yevxcyt6qb526x5 foreign key (id_usuario) references ms_usuarios.usuario;
alter table ms_usuarios.empleado add constraint FKt7vdal63o7rdoojoy7ywhjesh foreign key (id_usuario) references ms_usuarios.usuario;
alter table ms_usuarios.obra add constraint FKe6pqauh2107yp091rdhq78th1 foreign key (id_cliente) references ms_usuarios.cliente;
alter table ms_usuarios.obra add constraint FKjtg36cfq5imcy9u57c677w4lt foreign key (id_tipo_obra) references ms_usuarios.tipo_obra;
alter table ms_usuarios.usuario add constraint FKr9xk0brid147iaydo8j47o9p2 foreign key (id_tipo_usuario) references ms_usuarios.tipo_usuario;
...
alter table ms_usuarios.obra add constraint UK_3wa1bw3rpmbkbc5evlt9d2pc3 unique (descripcion);
...
alter table ms_pedidos.pedido add constraint FKmtnlpv8yx33wfgetcgeuatap0 foreign key (id_obra) references ms_usuarios.obra;
...
alter table ms_pagos.pago add constraint FK2fmwlqws6jmrycdhkn2bl8m0p foreign key (id_cliente) references ms_usuarios.cliente;
